...
def initCrawlerScraper(seed,max_profiles=500):
    ...
    while (nextLink): #enquanto existirem páginas seguintes ativas
        ...
        try:
            if driver.find_element_by_css_selector(".nextLink"): #tenta encontrar o botao next
                    ...
                    if "research output".lower() in a.text.lower(): # link "Research Output" para ver publicações
                        ...
                        if table != None:
                            for row in table.findAll('div', attrs={'class': 'result-container'}):
                                ...
                                print("\n")
                                pub_data.append(data) #Baixa os dados de cada publicação (Nome, URL, Autor e Data) e armazena essas informações na lista pub_data
