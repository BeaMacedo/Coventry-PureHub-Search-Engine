...
def search_ngrams_only(input_text, operator_val, search_type="publication", rank_by="Sklearn function"):
    ...
    if operator_val == 2:  # OPERADOR OR (igual à search_data)
        ...
        for phrase in phrases:
            ...
            # Verificar índices
            if len(words) == 2 and phrase in bigram_index.get(stem_word_file):
                pointer = bigram_index.get(stem_word_file)
            elif len(words) == 3 and phrase in trigram_index.get(stem_word_file):
                pointer = trigram_index.get(stem_word_file)
        ...
        for phrase in phrases:
            ...
            # Verificar índices
            if len(words) == 2 and phrase in bigram_index.get(stem_word_file):
                set1 = set(bigram_index.get(stem_word_file))
                pointer.extend(list(set1))  # Adiciona o conjunto set1 ao pointer
            elif len(words) == 3 and phrase in trigram_index.get(stem_word_file):
                set1 = set(trigram_index.get(stem_word_file))
                pointer.extend(list(set1))
...
def app():
    ...
    if st.session_state.mode == "general":
        ...
        if option == "Show all publications":
            ...
            if st.button("SEARCH"):
                output_data = search_Centre_data(
                    input_text,
...
def show_Centre_results(output_data, input_text=None, stem_lema=None):
    ...
    aa = 0
    # Ordenar corretamente considerando arrays numpy
    def get_ranking_value(item):
        val = item[1]
        return float(val[0] if isinstance(val, (list, np.ndarray)) else val)

    rank_sorting = sorted(output_data.items(), key=get_ranking_value, reverse=True)
    print(f"ranking_sorting:{rank_sorting}")
    ...
    for n_row, (id_val, ranking) in enumerate(rank_sorting):
        ...
        with cols[n_row % N_cards_per_row]:
            ...
            if isinstance(ranking, (list, np.ndarray)):
                display_rank = float(ranking[0])
            else:
                display_rank = float(ranking)

            st.markdown(f"Ranking: {display_rank:.4f}")
